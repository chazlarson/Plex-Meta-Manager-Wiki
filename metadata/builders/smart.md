...
To make a collection a Smart Label Collection you have to add the `smart_label` detail to your collection definition. It functions teo different ways. 

1. You can provide how you want it sorted from the Movies/Shows column of the [Sorts Table](#sort-options) below along with any other builder to make that collection a Smart Label Collection.
    ```yaml
    collections:
...
2. You can provide a whole `smart_filter` to determine exactly how you want the smart collection built, just make sure to include `lable: <<smart_label>>` to link it to the collection labels.
    ```yaml
    collections:
